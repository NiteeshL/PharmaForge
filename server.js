...
app.post('/getDrugDetails2', (req, res) => {
  const { drugName } = req.body;

  // Query the database for the quantity of the specified drug
  connection.query('SELECT Quantity FROM drugs WHERE Drug_name = ?', [drugName], (error, results) => {
      if (error) {
          console.error('Error querying drug details:', error);
          return res.status(500).json({ success: false, message: 'Internal server error' });
      }
